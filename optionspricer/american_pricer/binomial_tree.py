...
class BinomialTree:
    ...
    def _price(self):
        ...
        for j in range(self.steps-1, -1, -1):
            for i in range(j+1):
                if (self.type == 'call'):
                    option_tree[i, j] = max(stock_tree[i, j] - self.K, np.exp(-self.r*self.dt) * (self.p * option_tree[i, j+1] + (1-self.p) * option_tree[i+1, j+1]))
                else:
                    option_tree[i, j] = max(self.K - stock_tree[i, j], np.exp(-self.r*self.dt) * (self.p * option_tree[i, j+1] + (1-self.p) * option_tree[i+1, j+1]))
